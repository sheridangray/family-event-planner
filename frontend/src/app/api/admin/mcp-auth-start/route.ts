...
export async function POST(req: NextRequest) {
  try {
    // Check authentication - allow both family members
    ...
    const response = await fetch(`${BACKEND_API_URL}/api/admin/mcp-auth-start`, {
      method: 'POST',
      headers: {
        'x-api-key': API_KEY,
    ...
    console.error('MCP auth start API error:', error);
