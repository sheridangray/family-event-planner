...
class TaskScheduler {
  ...
  async runEventDiscovery(isManual = false) {
    ...
    try {
      ...
      discoveryRunId = await this.database.createDiscoveryRun(
      ...
      this.logger.info(
        `Running discovery #${discoveryRunId} with ${
          enabledScrapers.length
        } enabled scrapers: ${enabledScrapers.map((s) => s.name).join(", ")}`
      );
      ...
      for (const scraper of enabledScrapers) {
        ...
        try {
          // Configure days to scrape based on manual vs scheduled and scraper type
          const scrapingOptions = {};
          if (scraper.name === "cal-academy") {
            // Manual discovery: 3 days, Scheduled discovery: 30 days
            scrapingOptions.daysToScrape = isManual ? 3 : 30;
            this.logger.info(
              `Cal Academy will scrape ${scrapingOptions.daysToScrape} days (${
                isManual ? "manual" : "scheduled"
              })`
            );
          }

          const events = await this.scraperManager.scrapeSource(
            scraper.name,
            discoveryRunId,
            scrapingOptions
          );
          ...
          this.logger.error(
            `Error with scraper ${scraper.name}:`,
            error.message
          );
        ...
        try {
          ...
          `,
            [
              ...
              errorMessage,
              executionTime,
              new Date(scraperStartTime),
              new Date(),
            ]
          );
        } catch (statsError) {
          this.logger.error(
            `Failed to record stats for ${scraper.name}:`,
            statsError.message
          );
        }
      }
      ...
      if (allEvents.length > 0) {
        ...
        let sentCount = 0;
        const maxEventsToSend = isManual
          ? topEvents.length
          : config.discovery.eventsPerDayMax;

        this.logger.info(
          `${
            isManual ? "Manual" : "Scheduled"
          } discovery: sending up to ${maxEventsToSend} events for approval`
        );
        this.logger.info(
          `📊 Notification service status: unifiedNotifications=${!!this
            .unifiedNotifications}, smsManager=${!!this.smsManager}`
        );

        for (let i = 0; i < topEvents.length; i++) {
          const event = topEvents[i];
          this.logger.info(
            `📧 Processing event ${i + 1}/${topEvents.length}: "${
              event.title
            }" (${event.date})`
          );
          ...
          try {
            ...
            if (this.unifiedNotifications) {
              ...
              this.logger.info(
                `✅ Using unified notification service for: ${event.title}`
              );

              const shouldSend =
                isManual || (await this.unifiedNotifications.shouldSendEvent());
              this.logger.info(
                `🔍 Should send event? ${shouldSend} (isManual=${isManual})`
              );

              if (shouldSend) {
                this.logger.info(
                  `📤 Sending approval request for: ${event.title}`
                );
                await this.unifiedNotifications.sendEventForApproval(event);
                sentCount++;
                this.logger.info(
                  `✅ Successfully sent email for: ${event.title} (count: ${sentCount}/${maxEventsToSend})`
                );
              } else {
                this.logger.info(
                  `⏭️ Skipping event due to rate limiting: ${event.title}`
                );
              }
            } else if (this.smsManager) {
              // Fallback to SMS only if email notifications unavailable
              this.logger.warn(
                `📱 Falling back to SMS for: ${event.title} (email notifications unavailable)`
              );
              if (isManual || (await this.smsManager.shouldSendEvent())) {
                await this.smsManager.sendEventForApproval(event);
                sentCount++;
                this.logger.info(
                  `✅ Successfully sent SMS for: ${event.title} (count: ${sentCount}/${maxEventsToSend})`
                );
              }
            } else {
              this.logger.error(
                "❌ No notification service available - cannot send approval requests (smsManager and unifiedNotifications are both null)"
              );
              break;
            }
            ...
            if (sentCount >= maxEventsToSend) {
              this.logger.info(
                `🛑 Reached maximum events to send (${maxEventsToSend}), stopping`
              );
              break;
            }
            ...
            this.logger.error(
              `❌ Failed to send notification for "${event.title}" (ID: ${event.id}): ${emailError.message}`
            );
        ...
        this.logger.info(
          `Event discovery completed: ${allEvents.length} discovered, ${filteredEvents.length} filtered, ${sentCount} sent for approval`
        );
  ...
  async runSingleScraper(scraperName, isManual = true) {
    ...
    try {
      ...
      this.logger.info(
        `Running individual scraper discovery #${discoveryRunId} for ${scraperName}`
      );
      ...
      if (events.length > 0) {
        ...
        const maxEventsToSend = isManual
          ? topEvents.length
          : Math.min(5, config.discovery.eventsPerDayMax);

        this.logger.info(
          `${
            isManual ? "Manual" : "Scheduled"
          } single scraper: sending up to ${maxEventsToSend} events for approval`
        );

        for (const event of topEvents) {
          if (this.unifiedNotifications) {
            this.logger.info(
              `Using email-only notification service for: ${event.title}`
            );
            if (
              isManual ||
              (await this.unifiedNotifications.shouldSendEvent())
            ) {
              await this.unifiedNotifications.sendEventForApproval(event);
              sentCount++;
            }
            ...
            this.logger.error(
              "No notification service available - cannot send approval requests"
            );
            break;
          }
        ...
        this.logger.info(
          `Single scraper discovery completed for ${scraperName}: ${events.length} discovered, ${filteredEvents.length} filtered, ${sentCount} sent for approval`
        );
        ...
        try {
          ...
          this.logger.error(
            `Failed to record stats for ${scraperName}:`,
            statsError.message
          );
        }
        ...
        this.logger.info(
          `Single scraper discovery completed for ${scraperName}: no new events found`
        );
      ...
      errorMessage = error.message;
      this.logger.error(
        `Single scraper discovery failed for ${scraperName}:`,
        error.message
      );
      ...
      try {
        ...
        this.logger.error(
          `Failed to record failed stats for ${scraperName}:`,
          statsError.message
        );
      }
    ...
    const duration = Date.now() - startTime;
    this.logger.info(
      `Single scraper discovery for ${scraperName} took ${duration}ms`
    );
  }

  async runEventProcessing() {
    try {
      const discoveredEvents = await this.database.getEventsByStatus(
        "discovered"
      );

      if (discoveredEvents.length > 0) {
        const filteredEvents = await this.eventFilter.filterAndSort(
          discoveredEvents,
          {
            calendarChecker: this.calendarManager,
            database: this.database,
        ...
        this.logger.info(
          `Processed ${discoveredEvents.length} events: ${scoredEvents.length} remain viable`
        );
      }
    } catch (error) {
  ...
  async checkApprovalTimeouts() {
    try {
      ...
      if (this.unifiedNotifications) {
        ...
        this.logger.warn(
          "Using SMS for timeout checks (email notifications unavailable)"
        );
      ...
      if (timeoutCount > 0 || reminderCount > 0) {
        this.logger.info(
          `Approval timeout check: ${timeoutCount} timeouts, ${reminderCount} reminders sent`
        );
      }
    } catch (error) {
  ...
  async processRegistrations() {
    try {
      ...
      if (results.length > 0) {
        this.logger.info(
          `Registration processing: ${successCount} successful, ${failureCount} failed`
        );
        ...
        for (const result of results) {
          if (result.success) {
            try {
              ...
              this.logger.error(
                `Failed to create calendar event for ${result.event.title}:`,
                error.message
              );
            }
          }
    ...
    try {
      ...
      for (const event of bookedEvents) {
        try {
          ...
          this.logger.error(
            `Failed to sync calendar for ${event.title}:`,
            error.message
          );
        }
      }
      ...
      if (bookedEvents.length > 0) {
        this.logger.info(
          `Calendar sync completed: ${bookedEvents.length} events processed`
        );
      }
    } catch (error) {
...
🔄 System Health: ${stats.systemHealth}
      ...
      try {
        ...
        if (emailResult.success) {
          this.logger.info(
            `Daily report emailed to: ${emailResult.recipients.join(", ")}`
          );
        } else {
          this.logger.warn(
            `Email failed (${emailResult.reason}), but report saved to file`
          );
        }
      } catch (error) {
  ...
  async getDailyStats() {
    ...
    try {
      ...
      return {
        ...
        newToday: discovered.filter(
          (e) => new Date(e.created_at).toDateString() === today
        ).length,
        sentToday: proposed.filter(
          (e) => new Date(e.updated_at).toDateString() === today
        ).length,
        registeredToday: booked.filter(
          (e) => new Date(e.updated_at).toDateString() === today
        ).length,
        ...
        totalPendingCost: [...approved, ...booked].reduce(
          (sum, e) => sum + (e.cost || 0),
          0
        ),
  ...
  async runHealthCheck() {
    try {
      ...
      const issues = Object.entries(health).filter(
        ([key, value]) => !value.healthy
      );

      if (issues.length > 0) {
        this.logger.warn(
          `Health check issues: ${issues.map(([key]) => key).join(", ")}`
        );
      }
    } catch (error) {
  ...
  async checkScrapersHealth() {
    try {
      ...
      const recentlySuccessful = enabledScrapers.filter(
        (s) => s.total_runs > 0 && s.successful_runs / s.total_runs >= 0.5
      ).length;

      // Check if last discovery run was successful and recent (within 24 hours)
      const lastRunSuccessful =
        lastRun && lastRun.status === "completed" && !lastRun.error_message;
      const lastRunRecent =
        lastRun &&
      ...
      return {
        ...
        details: {
          ...
          scraperBreakdown: scraperStats.map((s) => ({
            ...
            avgExecutionTime: s.avg_execution_time
              ? Math.round(s.avg_execution_time)
              : 0,
            lastRun: s.last_run,
          })),
        },
    ...
    try {
      ...
      try {
        if (
          ...
          typeof this.calendarManager.hasConflict === "function"
        ) {
          ...
          healthChecks.gmail.error =
            "Calendar manager not initialized or missing methods";
        }
      } catch (error) {
      ...
      try {
        if (
          this.smsManager &&
          typeof this.smsManager.shouldSendEvent === "function"
        ) {
          ...
          healthChecks.twilio.error =
            "SMS manager not initialized or missing methods";
        }
      } catch (error) {
      ...
      const overallHealthy =
        healthChecks.gmail.healthy && healthChecks.twilio.healthy;
    ...
    try {
      ...
      try {
        ...
        if (emailStats.rows[0] && emailStats.rows[0].total > 0) {
          const successRate =
            (emailStats.rows[0].sent / emailStats.rows[0].total) * 100;
  ...
  async checkDatabasePerformanceHealth() {
    try {
      ...
      console.log(
        `Database performance check: basic=${basicQueryTime}ms, complex=${complexQueryTime}ms, healthy=${isHealthy}`
      );
  ...
  async calculateSystemHealthScore() {
    try {
      ...
      this.logger.error(
        "Error calculating system health score:",
        error.message
      );
  ...
  getDiscoveryProgress() {
    return {
      ...
      progress:
        this.discoveryProgress.totalScrapers > 0
          ? Math.round(
              (this.discoveryProgress.completedScrapers /
                this.discoveryProgress.totalScrapers) *
                100
            )
          : 0,
    };
