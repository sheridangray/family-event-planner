...
class UnifiedNotificationService {
  ...
  async init() {
    if (this.userId) {
      this.logger.info(
        `Initializing email notification service for user ${this.userId}...`
      );
      await this._initializeUserEmailManager(this.userId);
    } else {
      this.logger.info(
        "Initializing email-only notification service (single-user mode)..."
      );
  ...
  async _initializeSingleUserEmailManager() {
    try {
      ...
      if (!senderId) {
        throw new Error(
          `No user found in database for email: ${senderEmail}. Please ensure user exists in database.`
        );
      }

      this.logger.info(
        `Found sender user ID: ${senderId} for email: ${senderEmail}`
      );
      ...
      this.logger.info(
        `Single-user email notifications available (sending from user ${senderId})`
      );
    } catch (error) {
      this.logger.error(
        "Single-user email initialization failed:",
        error.message
      );
      this.emailAvailable = false;
      throw new Error(
        "Email notification initialization failed - no notification methods available"
      );
    }
  }
  ...
  async _initializeUserEmailManager(userId) {
    try {
      ...
      this.logger.error(
        `Email initialization failed for user ${userId}:`,
        error.message
      );
      throw error;
    }
  ...
  async sendEventForApproval(event, userId = null) {
    try {
      this.logger.info(
        `📧 UnifiedNotificationService: Sending approval request for event: ${
          event.title
        } (user: ${userId || this.userId || "default"})`
      );
      this.logger.info(`📧 Email service available: ${this.emailAvailable}`);

      if (!this.emailAvailable) {
        this.logger.error(
          `❌ Email notifications not available for: ${event.title}`
        );
      ...
      this.logger.info(
        `📤 Calling emailManager.sendEventForApproval for: ${event.title}`
      );
      const result = await emailManager.sendEventForApproval(event);
      this.logger.info(
        `✅ Email notification sent successfully for: ${event.title}`
      );
      this.logger.info(`📊 Email result:`, result);
      return { ...result, method: "email", userId: userId || this.userId };
    } catch (error) {
      this.logger.error(
        `❌ UnifiedNotificationService error sending approval request for "${event.title}" (ID: ${event.id}): ${error.message}`
      );
  ...
  async processApprovedEvent(
    ...
    try {
      ...
      if (method !== "email" && method !== null) {
        this.logger.warn(
          `Unsupported approval method '${method}', treating as email`
        );
      }
  ...
  async sendEventForApprovalToAllUsers(event) {
    try {
      ...
      const authenticatedUsers = userStatuses.filter(
      ...
      for (const user of authenticatedUsers) {
        try {
          this.logger.info(
            `Sending event ${event.title} for approval to user ${user.userId} (${user.email})`
          );
          ...
          this.logger.error(
            `Failed to send event ${event.title} to user ${user.userId}:`,
            error.message
          );
          results.push({
            userId: user.userId,
      ...
      this.logger.error(
        "Error sending event for approval to all users:",
        error.message
      );
      throw error;
    }
  ...
  async checkTimeouts() {
    ...
    if (this.emailAvailable) {
      try {
        ...
        results.email = await emailManager.checkTimeouts();
      } catch (error) {
  ...
  async sendReminders() {
    ...
    if (this.emailAvailable) {
      try {
        const emailManager = await this._getEmailManager();
        const emailReminders = await emailManager.sendReminders();
   ...
   */
  async shouldSendEvent(method = "email") {
    if (method !== "email" && method !== null) {
    ...
    try {
      ...
      this.logger.error("Error checking shouldSendEvent:", error.message);
  ...
  async getDailyEventCounts() {
    try {
      const emailManager = await this._getEmailManager();
      return {
        email: emailManager.dailyEventCount,
        activeMethod: "email",
      };
    } catch (error) {
      this.logger.error("Error getting daily event counts:", error.message);
      return {
        email: 0,
        activeMethod: "email",
      };
